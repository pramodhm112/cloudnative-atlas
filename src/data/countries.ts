/**
 * Full ISO 3166-1 alpha-2 country list with E.164 dial codes.
 *
 * Dial codes follow ITU-T E.164 (1–3 digits). For the North American Numbering
 * Plan, all member territories share `+1`; the area code is part of the local
 * number, which matches how international forms typically work.
 *
 * Server-side validation MUST verify the submitted ISO code is in
 * COUNTRY_CODES and the dial code is in DIAL_CODES — never accept arbitrary
 * strings.
 */
...
export interface Country {
  ...
  name: string;
  /** E.164 dial code without the leading + (1–3 digits) */
  dial: string;
}
...
/** Pre-built lookup sets for fast O(1) server-side validation. */
export const COUNTRY_CODES: ReadonlySet<string> = new Set(COUNTRIES.map((c) => c.code));
export const DIAL_CODES: ReadonlySet<string> = new Set(COUNTRIES.map((c) => c.dial));

/** Sorted by name for stable display in the dropdown. */
